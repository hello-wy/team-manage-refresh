...
“Team 成员管理”按标准、高级分别显示剩余席位、已购和已加入数量，有邀请或待确认操作时显示对应数量。
余额读取订阅接口的 `seat_capacity`，标准类型为 `default`，高级类型为 `prolite`。
本地配置的“最大成员数”不作为已购配额使用。
工作台和福利池的成员数显示为“已加入成员数 / 总席位数”：邀请不计入分子，分母为已购标准与高级席位之和。
Team 列表表头简写为“成员/席位”，悬停可查看完整含义；右上角不再显示每页数量和条目总数，底部分页仍可调整每页数量。
该数据在导入、同步和刷新成员时更新；尚未获取或配额读取失败时显示 `—`，不会回退到配置上限。

成员列表操作列的钥匙图标打开独立的“ChatGPT 授权”弹窗，支持已加入和待接受邀请的成员：获取授权链接后，使用该成员邮箱登录，粘贴完整回调 URL，再点击“解析并自动填充”。关闭弹窗或按 Esc 返回成员列表；下载、删除与撤回也使用带悬停提示的图标按钮。
授权链接 15 分钟内有效且只能使用一次；服务端核对登录邮箱并加密保存成员自己的凭证，列表不会返回 Token。
点击“判定邀请结果”会读取上游实时成员列表。授权不等于接受邀请；仍在邀请中的成员需要先接受邀请，授权时选择当前 Team 工作区。
授权解析及入组判定完成后，成员列表、席位余额和 Team 列表人数立即同步。授权成功但仍待接受邀请时，授权弹窗每 5 秒自动检查一次，关闭弹窗停止检查；确认入组后显示绿色完成提示和“邀请已完成”气泡，无需手动刷新。
只有已授权且已加入当前 Team 的成员可以导出。导出前会通过 Team 管理凭证重新读取实时成员列表，并校验成员授权仍有效且邮箱一致；未授权、仍在邀请中、已被移除或授权失效时均拒绝导出。
“导出 sub2api JSON”生成 `sub2api-data` v1 文件，可在 [sub2api](https://github.com/Wei-Shaw/sub2api) 的账号管理 → 数据导入中使用；文件含该成员的敏感授权凭证，请妥善保管。
Team 所有者使用已保存的母号 AT/RT：读取成员列表时核对所有者邮箱与 Token 身份，并将授权及加密的 sub2api JSON 同步到成员管理。Team 列表的推送按钮及批量推送会再次核验实时入组状态，然后按系统中心的 sub2api 分组配置导入母号；缺少 RT 时需先更新 Team 凭证。
此功能只判定邀请结果，不会自动接受邀请。已有数据库在应用启动时自动创建 `member_authorizations` 表。

- 剩余余额会扣除待接受邀请和未确认操作的预留。待生效的降级不会提前释放原席位。
- 余额不足或读取失败时，保存和邀请均被禁用；服务端提交前会再次检查，批量邀请超出对应类型余额时整批拒绝。
- 未确认操作保存在 `team_seat_holds` 表，重启不丢失；上游成员或邀请列表确认后自动核销预留。请求明确失败或撤回成功时释放对应预留。
- 点击“刷新余额”重新查询。服务按单实例部署，同一工作区的操作串行执行；在官方页面进行的变更仍以提交时的上游结果为准。
